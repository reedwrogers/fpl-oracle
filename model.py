...
def find_latest_gameweek():
    ...
    # Predict the upcoming gameweek from the FPL API. Pre-season this points at
    # GW 1 (already generated by preseason.py); in-season it advances as each
    # gameweek finishes.
    try:
        url = "https://fantasy.premierleague.com/api/bootstrap-static/"
        events = pd.DataFrame(requests.get(url).json()["events"])
        upcoming = int(events.loc[events["is_next"], "id"].iloc[0])
    except Exception:
        upcoming = None
